...
public class ApiClient {
    ...
    public ApiClient(boolean validating) {
        ...
        setUserAgent("CDS Client/2.0.0/java");
    ...
    /**
     * Execute HTTP call and deserialize the HTTP response body into the type determined by return type resolver.
     *
     * @param returnTypeResolver The return type resolver
     * @param <T>                The return type corresponding to (same with) returnType
     * @param call               Call
     * @return ApiResponse object containing response status, headers and data, which is a Java object
     * deserialized from response body and would be null when returnType is null.
     * @throws ApiException If fail to execute the call
     */
    public <T> ApiResponse<T> execute(Call call, ReturnTypeResolver returnTypeResolver) throws ApiException {
        try {
            Response response = call.execute();
            return handle(response, returnTypeResolver.resolve(response));
        } catch (IOException e) {
            throw new ApiException(e);
        }
    }
